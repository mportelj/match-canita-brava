...
def leer_datos():
    ...
    try:
        ...
        # 2. Asegurar que temporada sea texto puro y sin espacios
        df['temporada'] = df['temporada'].astype(str).str.replace('.0', '', regex=False).str.strip()
        # 3. Forzar números en resultados
...
if menu == "Inicio":
    ...
    if not df.empty:
        # Filtramos por temporada 2026 asegurando el formato
        ...
        if not df_2026.empty:
            ...
            for _, r in resumen.iterrows():
                # Solo sumamos si el partido tiene algún punto guardado
...
# --- JUGAR / EDITAR ---
    ...
    if 'game' not in st.session_state:
        ...
        if g['logs']:
            ...
            st.markdown(f"<div style='background:#fff; border:1px solid #ccc; padding:10px; border-radius:10px; text-align:center;'><b>MATCH HOY:</b> <span style='color:{COLOR_A}'>{match_a:g}</span> vs <span style='color:{COLOR_B}'>{match_b:g}</span></div>", unsafe_allow_html=True)
