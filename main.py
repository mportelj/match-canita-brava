...
def leer_datos():
    try:
        ...
        # ttl=0 es CLAVE para que no lea datos viejos de la memoria
...
def guardar_partida(df_partida):
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        # Limpiamos caché de Streamlit antes de leer para asegurar frescura
        st.cache_data.clear()
        df_existente = leer_datos()
        
        # Forzamos ID como string y quitamos espacios
        id_nuevo = str(df_partida["id"].iloc[0]).strip()
        
        if not df_existente.empty:
            df_existente['id'] = df_existente['id'].astype(str).str.strip()
            # MANTENEMOS solo las filas que NO coinciden con nuestro ID actual
            df_limpio = df_existente[df_existente["id"] != id_nuevo].copy()
            # Añadimos la nueva versión de la partida
            df_final = pd.concat([df_limpio, df_partida], ignore_index=True)
        else:
            df_final = df_partida

        # SEGURIDAD EXTRA: Si por algún motivo falló el filtro anterior, 
        # eliminamos duplicados por ID quedándonos con el último (el más reciente)
...
def calcular_puntos_hoyo(s1, s2, s3, s4, hoyo_num):
    ...
    # 2. Bonus Birdie/Eagle (Se suma al marcador del equipo)
    for s in [s1, s2]:
        if 0 < s <= par - 2: pa += 2.0
...
if menu == "Inicio":
    ...
    if 'game' not in st.session_state:
        ...
        if st.button("💾 Grabar Hoyo", type="primary", disabled=ya_guardado, use_container_width=True):
        ...
        if g['logs']:
            ...
            t_b = sum(v['pts'][1] for v in g['logs'].values())
            st.divider()
            st.markdown(f"<h2 style='text-align:center;'>Marcador: {int(t_a)} - {int(t_b)}</h2>", unsafe_allow_html=True)
